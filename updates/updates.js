/* /updates/updates.js
   Filters, release detail toggles, feedback collapse, hash sync and keyboard support.
*/
...
(() => {
  ...
  const activeLabel = $("#updatesActiveLabel");
  ...
  function setActiveChip(key, { focus = false } = {}) {
    ...
    if (activeLabel) {
      const activeChip = chips.find((c) => c.getAttribute("data-filter") === key);
      activeLabel.textContent = activeChip ? activeChip.textContent.trim() : "All";
    }
    if (focus) {
