(() => {
  ...
  function toolExamples(slug, config, tool) {
    ...
    const map = {
      ...
      "json-formatter": [
        ...
        { label: "Config", values: { json: "{\"theme\":{\"accent\":\"#6366f1\"},\"features\":[\"tools\",\"guides\"]}" } },
  ...
  const configs = {
    ...
    "meta-tags": {
      ...
      run(v) { const title = String(v.title || ""), desc = String(v.description || ""); return result("Meta tags checked.", [metric("Title chars", title.length), metric("Description chars", desc.length), metric("Title status", title.length <= 60 ? "Good" : "Long"), metric("Description status", desc.length <= 160 ? "Good" : "Long")], `<title>${title}</title>\n<meta name="description" content="${desc.replace(/"/g, "&quot;")}" />`); }
    },
    "serp-preview": {
      ...
      run(v) { return result("SERP preview text ready.", [metric("Title", String(v.title || "").length), metric("Description", String(v.description || "").length), metric("URL", "Shown"), metric("Intent", "Check")], `${v.title}\n${v.url}\n${v.description}`); }
    },
    "keyword-density": {
      ...
      run(v) { const ws = words(v.text).map((w) => w.toLowerCase()).filter((w) => w.length > 2); const map = new Map(); ws.forEach((w) => map.set(w, (map.get(w) || 0) + 1)); const rows = [...map].sort((a, b) => b[1] - a[1]).slice(0, 12); return result("Keyword frequency calculated.", [metric("Words", ws.length), metric("Unique", map.size), metric("Top term", rows[0]?.[0] || "-"), metric("Top density", rows[0] ? `${((rows[0][1] / ws.length) * 100).toFixed(1)}%` : "0%")], rows.map(([k, n]) => `${k}: ${n} (${((n / ws.length) * 100).toFixed(1)}%)`).join("\n")); }
    },
    "slug-generator": {
      sample: "How to Write Better Meta Titles in 2026",
      fields: [field("text", "Title", "text", "How to Write Better Meta Titles in 2026", true)],
      run(v) { const slug = String(v.text || "").toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80); return result("SEO slug generated.", [metric("Length", slug.length), metric("Words", slug.split("-").filter(Boolean).length), metric("Status", slug.length <= 60 ? "Good" : "Long"), metric("Separator", "Hyphen")], slug); }
    },
    "json-formatter": {
    ...
    "url-encoder": {
      ...
      run(v) { const t = String(v.text || ""); return result("URL value encoded and decoded.", [metric("Original", t.length), metric("Encoded", encodeURIComponent(t).length), metric("Mode", "Both"), metric("Copy", "Ready")], `Encoded:\n${encodeURIComponent(t)}\n\nDecoded:\n${decodeURIComponent(t)}`); }
    },
    "base64": {
      sample: "Clickoz premium tools",
      fields: [field("text", "Text or Base64", "textarea", "Clickoz premium tools", true)],
      run(v) { const t = String(v.text || ""); let decoded = ""; try { decoded = atob(t); } catch (_) { decoded = "(input is not valid Base64)"; } return result("Base64 encode/decode ready.", [metric("Input chars", t.length), metric("Encoded chars", btoa(unescape(encodeURIComponent(t))).length), metric("Decoded", decoded.startsWith("(") ? "No" : "Yes"), metric("Copy", "Ready")], `Encoded:\n${btoa(unescape(encodeURIComponent(t)))}\n\nDecoded attempt:\n${decoded}`); }
    },
    "entity-encoder": {
    ...
    "utm-builder": {
      ...
      run(v) { const u = new URL(normUrl(v.url)); u.searchParams.set("utm_source", v.source || "source"); u.searchParams.set("utm_medium", v.medium || "medium"); u.searchParams.set("utm_campaign", v.campaign || "campaign"); return result("UTM link built.", [metric("Source", v.source || "-"), metric("Medium", v.medium || "-"), metric("Campaign", v.campaign || "-"), metric("URL", "Ready")], u.toString()); }
    },
  ...
  function fallbackConfig(slug) {
    ...
    return {
      ...
      run(v) {
        ...
        return result(`${tool.title} ready.`, [metric("Score", `${scoreHook(v.input)}/100`), metric("Platform", v.platform), metric("Sections", 4), metric("Next", "Copy/refine")], output);
      }
    };
  }
  ...
  function renderResult(root, res) {
    ...
    else out.textContent = res.output || "";
